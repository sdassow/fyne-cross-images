...
BUILD_ARGS += --build-arg FYNE_CROSS_IMAGES_VERSION=${VERSION}
BUILD_ARGS += --build-arg FYNE_CROSS_REPOSITORY=${REPOSITORY}
...
.android: .base android/Dockerfile
	@$(RUNNER) build ${BUILD_ARGS} -f ${CURDIR}/android/Dockerfile -t ${REPOSITORY}:${VERSION}-android .
	@$(RUNNER) tag ${REPOSITORY}:${VERSION}-android ${REPOSITORY}:android
	@touch .android

darwin: .darwin
.darwin: .base darwin/Dockerfile
	@$(RUNNER) build ${BUILD_ARGS} -f ${CURDIR}/darwin/Dockerfile -t ${REPOSITORY}:${VERSION}-darwin .
...
.darwin-sdk-extractor: .base darwin-sdk-extractor/Dockerfile
	@$(RUNNER) build ${BUILD_ARGS} -f ${CURDIR}/darwin-sdk-extractor/Dockerfile -t ${REPOSITORY}:${VERSION}-darwin-sdk-extractor .
...
.freebsd-base: .base freebsd/base/Dockerfile
	@$(RUNNER) build ${BUILD_ARGS} -f ${CURDIR}/freebsd/base/Dockerfile -t ${REPOSITORY}:${VERSION}-freebsd-base .
...
.freebsd-amd64: .freebsd-base freebsd/amd64/Dockerfile
	@$(RUNNER) build ${BUILD_ARGS} -f ${CURDIR}/freebsd/amd64/Dockerfile -t ${REPOSITORY}:${VERSION}-freebsd-amd64 .
...
.freebsd-arm64: .freebsd-base freebsd/arm64/Dockerfile
	@$(RUNNER) build ${BUILD_ARGS} -f ${CURDIR}/freebsd/arm64/Dockerfile -t ${REPOSITORY}:${VERSION}-freebsd-arm64 .
...
.linux: .base linux/Dockerfile
	@$(RUNNER) build ${BUILD_ARGS} -f ${CURDIR}/linux/Dockerfile -t ${REPOSITORY}:${VERSION}-linux .
...
android-push:
	@$(RUNNER) buildx build --pull --push ${BUILD_ARGS} -f ${CURDIR}/android/Dockerfile -t ${REPOSITORY}:${VERSION}-android -t ${REPOSITORY}:android .
